/*1294701619,176820409*/

if (window.CavalryLogger) {
	CavalryLogger.start_js( [ "tVaAM" ]);
}

if (!window.skipDomainLower
		&& navigator
		&& navigator.userAgent
//		&& document.domain.toLowerCase().match(/(^|\.)facebook\..*/)
		&& !(parseInt(
				(/Gecko\/([0-9]+)/.exec(navigator.userAgent) || []).pop(), 10) <= 20060508))
	document.domain = window.location.hostname.replace(/^.*(facebook\..*)$/i,
			'$1');
...
function bind(c, b) {
	...
	return function() {
		var e = c || (this == window ? false : this), d = a
...
function hasArrayNature(a) {
	return (!!a && (typeof a == 'object' || typeof a == 'function')
			&& ('length' in a) && !('setInterval' in a) && (Object.prototype.toString
			.call(a) === "[object Array]"
			|| ('callee' in a) || ('item' in a)));
}
function $A(b) {
	if (!hasArrayNature(b))
		return [ b ];
...
function eval_global(c) {
	if ('string' != typeof (c)) {
		throw new Error(
				'JS sent to eval_global is not a string.  Only strings ' + 'are permitted.');
	} else if ('' == c)
		return;
...
copy_properties(Arbiter, {
	...
	inform : function(i, c, b) {
		...
		for ( var e = 0; e < k.length; e++) {
			...
			} else if (i in a._events)
				a._events[i]._stateful = false;
			window.ArbiterMonitor
					&& ArbiterMonitor.getInstance(a).log('event', i, c);
			var g;
			if (a._index[i]) {
				var j = $A(a._index[i]);
			...
			window.ArbiterMonitor
					&& ArbiterMonitor.getInstance(a).log('done', i, c);
			h[i] = g;
		}
		return l ? h : h[k[0]];
	},
...
var Bootloader = window.Bootloader = (window.Bootloader && window.Bootloader.realBootloader) ? window.Bootloader
		: (function(a) {
			return {
				realBootloader : true,
				...
				loadResources : function(j, b, i, m) {
					...
					if (i) {
						...
						if (!window.ResourceBundler || !e) {
							...
							ResourceBundler
									.dropLocalResources(this._earlyResources);
						for ( var f in this._requested)
							if (!(f in this._permanent) && !(f in g)
									&& !(f in this._earlyResources))
						...
						if (!this._requested[k.name]) {
							...
							if (k.hash) {
								...
								window.CavalryLogger
										&& CavalryLogger.getInstance()
												.measureResources(k, m);
							}
						}
					}
					if (b)
				...
				_fetchWithIframe : function(d) {
					...
					if (!this._iframe) {
						...
						c.onload = bind(null, Arbiter.inform, e, true,
				...
				requestResource : function(j, g, e) {
					...
					if (j == 'js') {
						...
						if (window.Env && Env.use_css_import_in_ie
				...
				_startCSSPoll : function(e) {
					var d = 'bootloader_' + e.replace(/[^a-z0-9]/ig, '_');
					var b = bagofholding;
					function c(f, i, g, h) {
						h.deferUntil(f, i, false, b.curry(g));
					}
					c(
							function() {
							...
							function() {
								...
								c(
										function() {
											var h = '42';
											var g;
											return f.offsetHeight == h
													|| f.currentStyle
													&& f.currentStyle.height == h + 'px'
													|| window.getComputedStyle
													&& (g = document.defaultView
															.getComputedStyle(
																	f, null))
													&& g
															.getPropertyValue('height') == h + 'px';
										}, Bootloader._CSS_POLL_EXPIRATION,
										"CSS timeout", function() {
											Bootloader.done( [ e ], true);
					...
					if (!c) {
						...
						Arbiter.inform(Arbiter.BOOTLOAD, e,
								Arbiter.BEHAVIOR_EVENT);
					}
					for ( var b = 0; b < f.length; ++b) {
						var d = f[b];
						Arbiter.inform(Arbiter.BOOTLOAD + '/' + d, true,
								Arbiter.BEHAVIOR_STATE);
					}
				},
				...
				_unloadResource : function(e) {
					if (e in this._cssLinkMap) {
						...
						Arbiter.inform(Arbiter.BOOTLOAD + '/' + e, null,
								Arbiter.BEHAVIOR_STATE);
					}
				},
				...
				loadEarlyResources : function(d) {
					this.setResourceMap(d);
					var c = [];
					for ( var b in d)
						c.push(this._resources[b]);
					this.loadResources(c);
					for ( var b in d) {
						var e = this._resources[b];
						if (!e.permanent)
							this._earlyResources[e.name] = e;
					}
				},
				_requested : {},
				_permanent : {},
...
if (window == window.top)
	window.Log = (function() {
		var j = '_e_', k = (window.name || '').toString();
		k = (k.length == 7 && j == k.substr(0, 3)) ? k.substr(3)
				: (window.name = j + window._EagleEyeSeed).substr(3);
		var h = j + k + '_', f = new Date(+new Date() + 604800000)
				.toGMTString(), d = window.location.hostname.replace(
				/^.*(facebook\..*)$/i, '$1'), e = '; expires=' + f
				+ ';path=/; domain=' + d, c = 0, g = false, i = [];
		var b = function(l) {
			return h + (c++) + '=' + encodeURIComponent(l) + e;
		};
		var a = function(m) {
			var n = (document.cookie.search(j) >= 0);
			while (i.length > 0) {
				var o = b(i[0]);
				if (n
						&& ((document.cookie.length + o.length) > 3950 || document.cookie
								.split(';').length > 19))
					break;
				document.cookie = o;
				n = true;
			...
			if (!!m
					|| !g
					&& n
					&& ((document.cookie.length > 2500 || document.cookie
							.split(';').length > 15)) && window.Arbiter
					&& window.OnloadEvent && Arbiter.query(OnloadEvent.ONLOAD)) {
				var l = new Image();
				g = true;
				l.onload = function() {
					g = false;
				...
				if (window.Env && Env.tracking_domain) {
					host = Env.tracking_domain;
				} else
					host = '';
				l.src = host + '/ajax/nectar.php?asyncSignal='
						+ (Math.floor(Math.random() * 10000) + 1) + '&'
						+ (!m ? '' : 's=') + (+new Date());
			}
		};
		return function(p, l, n) {
			var q = [ k, +new Date(), p ].concat(l);
			q.push(q.length);
			for ( var m = 0; m < q.length; m++)
				if (typeof q[m] == 'string') {
					q[m] = '"' + q[m].replace(/\"/g, '\\"').replace(/\n/g,
							'\\n') + '"';
				} else if (q[m] === null)
					q[m] = 'null';
			var o = '[' + q.join(',') + ']';
...
function set_ue_cookie(a) {
	document.cookie = "act=" + encodeURIComponent(a) + "; path=/; domain="
			+ window.location.hostname.replace(/^.*(\.facebook\..*)$/i, '$1');
}
var user_action = (function() {
	var j = (!window.ArbiterMonitor) ? 'r' : 'a', l = 0, f = 0, k, d, e, m, i, g, b, c = function() {
		if (!!g) {
			var o = {
				profile_minifeed : 1,
				info_tab : 1,
			...
			for ( var n = g; n && n != document.body; n = n.parentNode) {
				if (!n.id || typeof n.id !== 'string')
					continue;
				if (n.id.substr(0, 8) == 'pagelet_')
					return n.id.substr(8);
				if (n.id.substr(0, 8) == 'box_app_')
					return n.id;
				if (o[n.id])
					return n.id;
			}
		...
		set_ue_cookie(n);
		if (j == 'a') {
			ArbiterMonitor.initUE(n);
			e = ArbiterMonitor.get_intern_ref(g);
		}
		window.Log
				&& Log('act', [
						m,
						...
						e || c(g),
						j,
						window.URI ? URI.getRequestURI(true, true)
		...
		if (!q && o)
			q = o.getTarget();
		if (!!q) {
			d = (q.getAttribute && q.getAttribute('ajaxify') || q.action
					|| q.href || q.name);
			g = q;
		}
		if (!!n && !b)
...
CSS = window.CSS
		|| {
			...
			removeClass : function(b, a) {
				b = $(b);
				b.className = b.className.replace(new RegExp(
						'(^|\\s)' + a + '(?:\\s|$)', 'g'), '$1');
				return b;
			},
...
!function() {
	...
	b.onsubmit = function(d) {
		...
		Bootloader.loadComponents('dom-form', function() {
			window.Form && Form.bootstrap(e, f);
		});
		return false;
	};
}();
Function.prototype.extend = function(a) {
	if (!Metaprototype._arbiterHandle)
		Metaprototype._arbiterHandle = Arbiter.subscribe(Arbiter.BOOTLOAD,
...
copy_properties(Metaprototype, {
	...
	_queue : function(b, c) {
		...
		var a = Arbiter.registerCallback(bind(Metaprototype,
				Metaprototype._apply, b, c), [
				Arbiter.FUNCTION_EXTENSION + '/' + c, Arbiter.BOOTLOAD ]);
		if (a !== null)
		...
		for ( var a in this._pending)
			if (!!window[a]) {
				...
				if (!window[a].__class_extending) {
					Arbiter.inform(Arbiter.FUNCTION_EXTENSION + '/' + a, true,
							Arbiter.BEHAVIOR_STATE);
				} else
	...
	_apply : function(a, c) {
		...
		b.parent = d;
		if (!!a.__class_name)
			Arbiter.inform(Arbiter.FUNCTION_EXTENSION + '/' + a.__class_name,
					true, Arbiter.BEHAVIOR_STATE);
	}
});
...
var ua = {
	...
	_populate : function() {
		if (ua._populated)
			return;
		ua._populated = true;
		var a = /(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera(?:.+Version.|.)(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))/
...
function _bootstrapEventHandlers() {
	...
	if (a.addEventListener) {
		...
		if (d.location.protocol == 'https:')
			b = '//:';
		a
				.write('<script onreadystatechange="if (this.readyState==\'complete\') {'
...
onload_callback = Arbiter.registerCallback(function() {
	window.CavalryLogger
			&& CavalryLogger.getInstance().setTimeStamp('t_onload');
...
domcontent_callback = Arbiter.registerCallback(function() {
	window.CavalryLogger
			&& CavalryLogger.getInstance().setTimeStamp('t_domcontent');
	Arbiter.inform(OnloadEvent.ONLOAD_DOMCONTENT_CALLBACK, true,
			Arbiter.BEHAVIOR_STATE);
...
function intl_ends_in_punct(a) {
	if (typeof a != 'string')
		return false;
	return a.match(new RegExp(intl_ends_in_punct.punct_char_class + '[' + ')"'
			+ "'" + '\u00BB' + '\u0F3B' + '\u0F3D' + '\u2019' + '\u201D'
			+ '\u203A' + '\u3009' + '\u300B' + '\u300D' + '\u300F' + '\u3011'
			+ '\u3015' + '\u3017' + '\u3019' + '\u301B' + '\u301E' + '\u301F'
			+ '\uFD3F' + '\uFF07' + '\uFF09' + '\uFF3D' + '\s' + ']*$'));
		...
		if (!(typeof a != 'object')) {
			...
			for ( var c in a) {
				if (intl_ends_in_punct(a[c])) {
					d = new RegExp('\{' + c + '\}'
							+ intl_ends_in_punct.punct_char_class + '*', 'g');
				} else
					d = new RegExp('\{' + c + '\}', 'g');
				var b = '';
				if (a[c][0] != '~')
...
InitialJSLoader = {
	...
	callback : Arbiter.registerCallback(function() {
		Arbiter.inform(InitialJSLoader.INITIAL_JS_READY, true,
				Arbiter.BEHAVIOR_STATE);
...
Event.__bubbleSubmit = function(a, event) {
	if (document.documentElement.attachEvent) {
		...
		while (b !== false && (a = a.parentNode))
			b = a.onsubmit ? a.onsubmit(event) : Event.__fire
					&& Event.__fire(a, 'submit', event);
		return b;
	}
};
function BigPipe(a) {
	...
	this._cavalry = (this.lid && window.CavalryLogger) ? CavalryLogger
			.getInstance(this.lid) : null;
	this._inst = this._cavalry
			&& (window._pagelet_profile || this._cavalry.isPageletProfiler());
...
copy_properties(BigPipe.prototype, {
	...
	_downloadJsForPagelet : function(a) {
		Bootloader.loadResources(a.js || [], bind(this, function() {
			...
			var c = bind(this, function() {
				...
				a.provides
						&& this.arbiter.inform(a.provides, true,
								Arbiter.BEHAVIOR_STATE);
			});
	...
	_downloadCssAndDisplayPagelet : function(c) {
		if (this._inst)
			this._cavalry.onPageletEvent('css', c.id);
		var b = bind(this, function() {
			var d = c.display_dependency || [];
			var f = [];
			for ( var e = 0; e < d.length; e++)
				f.push(d[e] + '_displayed');
			this.arbiter
					.registerCallback(this._displayPagelet.bind(this, c), f);
		});
		var a = c.css || [];
		if (this.isReplay) {
			...
			Bootloader.loadResources(a, b, false, c.id);
	},
	onPageletArrive : function(b) {
		...
		if (!this._phaseDoneCallbacks[c])
			this._phaseDoneCallbacks[c] = this.arbiter.registerCallback(
					this._onPhaseDone.bind(this), [ 'phase_complete_' + c ]);
		if (b.the_end)
		...
		this.arbiter.registerCallback(this._downloadCssAndDisplayPagelet.bind(
				this, b), [ 'phase_begin_' + c ]);
		var a;
		if (!this.jsNonBlock) {
			a = this.domContentEvt;
		} else
			a = b.id + '_displayed';
		this.arbiter.registerCallback(this._downloadJsForPagelet.bind(this, b),
				[ a ]);
		this.onloadCallback = this.arbiter.registerCallback(
				this.onloadCallback, [ 'pagelet_onload' ]);
		this.arbiter.registerCallback(this._phaseDoneCallbacks[c],
				[ b.id + '_displayed' ]);
		b.is_last
				&& this.arbiter.inform('phase_complete_' + c, true,
						Arbiter.BEHAVIOR_STATE);
		b.invalidate_cache
				&& b.invalidate_cache.length
	...
	_onPhaseDone : function() {
		...
		var a = bind(this, function() {
			...
			this.arbiter.inform('phase_begin_' + b, true,
					Arbiter.BEHAVIOR_STATE);
		});
		if (this.isReplay) {
			a();
		...
		if (this._currentPhase === this._lastPhase)
			this.arbiter.inform('pagelet_displayed_all', true,
					Arbiter.BEHAVIOR_STATE);
		this._currentPhase++;
	},
		...
		if (h) {
			...
			if ('focus' == e.type || 'focusin' == e.type) {
				if (f.value == h) {
					...
					if (g) {
						CSS.addClass(g, b);
						CSS.addClass(g, 'child_was_focused');
					}
				}
			} else if (f.value == '') {
				CSS.addClass(f, d);
...
!function() {
	document.documentElement.onkeydown = function(a) {
		...
		var c = a.keyCode == 13 && !a.shiftKey
				&& CSS.hasClass(b, 'enter_submit');
		if (c) {
			Bootloader.loadComponents( [ 'dom', 'input-methods' ], function() {
				if (!Input.isEmpty(b)) {
					var d = DOM.scry(b.form, '.enter_submit_target')[0]
							|| DOM.scry(b.form, '[type="submit"]')[0];
					d && d.click();
				}
			});
			return false;
