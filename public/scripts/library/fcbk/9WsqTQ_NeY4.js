/*1293473772,176820408*/

if (window.CavalryLogger) {
	CavalryLogger.start_js( [ "scZ\/B" ]);
}
...
function captchaRefresh(d, e, f, a, b) {
	...
	if (e)
		c.registration_page = true;
	new AsyncRequest().setURI('/captcha/refresh_ajax.php').setMethod('GET')
			.setReadOnly(true).setData(c).setStatusElement('captcha_throbber')
			.setHandler(function(i) {
				var g = ge('captcha');
				var h = ge('captcha_error_msg');
				if (g)
					DOM.setContent(g, HTML(i.getPayload().captcha));
				if (b && h)
					hide(h);
			}).send();
}
function tfbcaptcha_whats_this(a) {
	new ContextualDialog()
			.setContext(a)
			.setTitle(_tx("Security Check"))
			.setBody(
					'<div class="tfbcaptcha_whats_this_popup">' + _tx("This is a standard security test that we use to prevent spammers from creating fake accounts and spamming users.") + '</div>')
			.setButtons(Dialog.OK).show();
}
create_captcha = window.create_captcha
		|| function() {
			var a = {};
			if (Env.recaptcha_focus_on_load)
				a.callback = Recaptcha.focus_response_field;
			setTimeout(function() {
				Recaptcha.create("6LezHAAAAAAAADqVjseQ3ctG3ocfQs2Elo1FTa_a",
						"captcha", a);
			}, 0);
		};
function m_create_audio_captcha() {
	window.Env = {};
	setTimeout(function() {
		m_create_audio_captcha_helper(
				"6LezHAAAAAAAADqVjseQ3ctG3ocfQs2Elo1FTa_a", "captcha", {});
	}, 0);
}
...
function recaptcha_log_action(a) {
	new AsyncRequest().setURI('/ajax/captcha/recaptcha_log_actions.php')
			.setData( {
				action : a,
...
var Recaptcha = {
	...
	ajax_verify : function(a) {
		...
		var b = Recaptcha._get_api_server() + "/ajaxverify" + "?c="
	...
	_call_challenge : function(a) {
		if (!Recaptcha.audio_only)
			Recaptcha.fail_timer_id = setTimeout(
					Recaptcha.fail_timer_id == -1 ? "recaptcha_log_action('timeout'); create_captcha();"
							: "create_captcha();", 15000);
		var b = Recaptcha._get_api_server() + "/challenge?k=" + a
				+ "&ajax=1&xcachestop=" + Math.random();
		if ($('extra_challenge_params') != null)
			b += "&" + $('extra_challenge_params').value;
		Recaptcha._add_script(b);
	},
	...
	_finish_widget : function() {
		...
		if (!Recaptcha.audio_only) {
			...
			Recaptcha._set_challenge(c.challenge, 'audio');
		if (b.tabindex)
			a('captcha_response').tabIndex = b.tabindex;
		if (Recaptcha.widget)
	...
	reload : function(d) {
		...
		if (typeof (d) == "undefined")
			d = 'r';
		var e = c.server + "reload?c=" + c.challenge + "&k=" + c.site
				+ "&reason=" + d + "&type=" + b.type + "&lang="
				+ Env.recaptcha_lang;
		if (a('extra_challenge_params') != null)
			e += "&" + a('extra_challenge_params').value;
		b.should_focus = d != 't';
		b._add_script(e);
	},
	...
	_set_challenge : function(e, f) {
		...
		if (f == 'audio') {
			...
			a('recaptcha_image').innerHTML = "<img style='display:block;' height='57' width='300' src='"
					+ d + "'/>";
		}
		Recaptcha._css_toggle("recaptcha_had_incorrect_sol",
				"recaptcha_nothad_incorrect_sol", c.is_incorrect);
		Recaptcha._css_toggle("recaptcha_is_showing_audio",
				"recaptcha_isnot_showing_audio", f == 'audio');
		b._clear_input();
		if (b.should_focus)
			b.focus_response_field();
		b._reset_timer();
	},
	...
	getAudioCaptchaHtml : function() {
		...
		if (b._is_ie()) {
			...
			e = '<embed src="'
					+ g
					+ '" quality="high" bgcolor="#869ca7" width="0" height="0" name="audiocaptcha" align="middle" play="true" loop="false" quality="high" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://get.adobe.com/flashplayer" url="'
					+ g + '"></embed> ';
		var d = (Recaptcha.checkFlashVer() ? '<br/><a class="recaptcha_audio_cant_hear_link" href="#" onclick="Recaptcha.playAgain(); return false;">' + _tx("Play Again") + '</a>'
				: '')
				+ '<br/><a class="recaptcha_audio_cant_hear_link" target="_blank" href="'
				+ f + '">' + _tx("Can't hear this") + '</a>';
		return e + d;
	},
	...
	checkFlashVer : function() {
		...
		var g = (navigator.appVersion.toLowerCase().indexOf("win") != -1) ? true
				: false;
...
function captcha_whatsthis(a) {
	var b = '<a onclick="window.open(\'http://recaptcha.net/popuphelp/\',\'recaptcha_popup\',\'width=460,height=570,location=no,menubar=no,status=no,toolbar=no,scrollbars=yes,resizable=yes\')">' + 'ReCaptcha</a>';
	new ContextualDialog()
			.setContext(a)
			.setTitle(_tx("Security Check"))
			.setBody(
					'<div class="captcha_popup" style="padding: 5px;">'
							+ _tx("This is a standard security test that we use to prevent spammers from creating fake accounts and spamming users.")
							+ '<br/><br/>'
							+ _tx(
									"Our captchas are provided by {provider_link}.",
									{
										provider_link : b
...
function intl_set_cookie_locale(a, d, c) {
	...
	new AsyncRequest().setURI('/ajax/intl/save_locale_cookie_logging.php')
			.setData( {
				new_locale : a,
				old_locale : b,
				source : c
...
function FormTypeABTester(t) {
	...
	var w = function(za) {
		...
		if (o) {
			var zc = zd - o;
			if (zc >= 0 && ((zb >= a && zb <= i) || zb == h))
				if (zc < 400) {
					r[Math.floor(zc / 20)]++;
				} else if (zc < 1000) {
					q[Math.floor((zc - 400) / 60)]++;
				} else if (zc < 3000)
					p[Math.floor((zc - 1000) / 200)]++;
		}
		o = zd;
	};
	var x = function(zb) {
...
function reg_bootload(a, d, e, c, f) {
	var b = function(g) {
		Bootloader.loadComponents( [ 'reg-util', 'editor' ], function() {
			new RegKeyPressListen(f);
			new CaptchaBoxKeyPressListen(a, d, f);
			regform_focused(c);
		});
	};
	if (ge(e)) {
...
function useragent() {
	...
	var e = '\146\141\143\145\142\157\157\153';
	if (!new RegExp('(^|\.)' + e + '\.(' + Env.tlds.join('|')
			+ ')([^.]*)(:(\d+))?(\/|$)', 'i').test(document.location)) {
		...
		if (d && d.action) {
			...
			if (Env.ffver && Env.ffver != a)
				b.qm = d.action;
		}
	}
	if (b.qp || b.qm) {
		var f = document.location.protocol + '//www.' + e
				+ '.com/ajax/ua_callback.php';
		if (document.referrer)
			b.qr1 = document.referrer;
